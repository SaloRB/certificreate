...
**Certificreate** - a Next.js app. > TODO: one-line description of the problem it
solves. Fill this in from `blueprint/project-plan.md`, then re-run `/overview`.

Stack: Next.js 16 (App Router) + React 19 + TypeScript (strict) + Tailwind CSS v4,
package manager npm. No database, auth, or test runner yet.
...
exposed through tool-specific adapters. This project has the Claude Code adapter
only:

- Claude Code: `.claude/skills/<skill>/SKILL.md`

`AGENTS.md` stays because `CLAUDE.md` imports it, and other tools read it. If the
Codex adapter (`.agents/`) is added later, keep the matching skills in both
folders aligned when workflow behavior changes.
