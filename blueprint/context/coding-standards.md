...
> npm. There is no database, auth provider, component library, or test runner
> yet. Add those sections when those tools land in the project.
...
- Import with the `@/*` alias (mapped to the repo root), not deep relative paths
...
- Theme colors come from the CSS variables in `app/globals.css`; add new tokens there rather than hardcoding hex values in components
- No inline styles
- Dark mode currently follows `prefers-color-scheme`
- No component library installed. > TODO: pick one (shadcn/ui or similar) or state that components are hand-rolled

## Data

> TODO: no database, ORM, or auth provider is installed yet. Record the choice
> here (and in `project-plan.md`) before the first data-backed feature.

- Server components fetch data directly; client components go through Server Actions
- Validate all external input at the boundary before using it
- Never trust a client-supplied user or owner id; scope owned queries by the authenticated identity

## Error Handling

- Use try/catch in Server Actions
- Return `{ success, data, error }` pattern from actions
- Display user-friendly error messages in the UI (no toast library is installed yet)
- Never surface raw exception text or stack traces to the user
